...
def _generate_voucher_no():
    last = (
        db.session.query(ContraEntryMaster.voucher_no)
        .order_by(ContraEntryMaster.id.desc())
        .with_for_update()
        .first()
    )
    num = 0
    if last and last[0] and last[0].startswith("CV"):
        try:
            num = int(last[0][2:])
        except Exception:
            num = 0
    return "CV" + str(num + 1).zfill(5)
...
def _build_line_payload(line):
    return {
        ...
        "accountName":    line.account.bank_holder_name if line.account else None,
        "accountCode":    line.account.bank_code        if line.account else None,
        "accountType":    line.account.type             if line.account else None,
...
def _summary_key(dr_type, cr_type):
    if dr_type == "Bank" and cr_type == "Cash":
        return "bankToCash"
    if dr_type == "Cash" and cr_type == "Bank":
        return "cashToBank"
    if dr_type == "Bank" and cr_type == "Bank":
        return "bankToBank"
    if dr_type == "Cash" and cr_type == "Cash":
        return "cashToCash"
    return None
...
def create_contra_entry(data, user_id):
    try:
        ...
        if not allowed:
            return res("You are not authorized to create contra entries", [], 403)
        ...
        return res("Contra entry created", {
            "id":         entry.id,
            "voucherNo":  entry.voucher_no,
            "contraUuid": entry.contra_uuid,
        }, 201)
...
def get_contra_entry_list(data):
    try:
        project_code = data.get("projectCode")
        if not project_code:
            return res("projectCode required", [], 400)

        query = ContraEntryMaster.query.filter(
            ContraEntryMaster.project_code == project_code
        )

        if data.get("workflowStatus"):
            query = query.filter(ContraEntryMaster.workflow_status == data["workflowStatus"])

        if data.get("search"):
            term = f"%{data['search']}%"
            query = query.filter(ContraEntryMaster.voucher_no.ilike(term))

        if data.get("fromDate"):
            query = query.filter(ContraEntryMaster.entry_date >= data["fromDate"])

        if data.get("toDate"):
            query = query.filter(ContraEntryMaster.entry_date <= data["toDate"])

        rows = query.order_by(ContraEntryMaster.id.desc()).all()

        result = []
        summary = {"bankToCash": 0.0, "cashToBank": 0.0, "bankToBank": 0.0, "cashToCash": 0.0}

        for r in rows:
            dr_line = next((l for l in r.lines if l.dr_cr == "Dr"), None)
            cr_line = next((l for l in r.lines if l.dr_cr == "Cr"), None)

            debit_from = dr_line.account.bank_holder_name if dr_line and dr_line.account else None
            credit_to  = cr_line.account.bank_holder_name if cr_line and cr_line.account else None

            if r.workflow_status == "Approved":
                dr_type = dr_line.account.type if dr_line and dr_line.account else None
...
def get_contra_entry_details(entry_id):
    try:
        entry = ContraEntryMaster.query.get(entry_id)
        if not entry:
            return res("Contra entry not found", [], 404)
        return res("Contra entry details fetched", _build_detail_payload(entry), 200)
    except Exception as e:
        return res(str(e), [], 500)
...
def edit_contra_entry(entry_id, data, user_id):
    try:
        entry = ContraEntryMaster.query.get(entry_id)
        if not entry:
            return res("Contra entry not found", [], 404)
        if entry.locked:
            return res("Contra entry is locked and cannot be edited", [], 400)
        ...
        if not allowed:
            return res("You are not authorized to edit this contra entry", [], 403)
        ...
        ContraEntryLine.query.filter_by(contra_id=entry.id).delete()
        db.session.flush()
        ...
        return res("Contra entry updated", {"id": entry.id, "voucherNo": entry.voucher_no}, 200)
...
def submit_contra_entry(entry_id, user_id):
    try:
        entry = ContraEntryMaster.query.get(entry_id)
        if not entry:
            return res("Contra entry not found", [], 404)
        if entry.workflow_status not in ("Draft", "Reback"):
            return res("Contra entry already submitted", [], 400)
        if not entry.lines:
            return res("Contra entry has no lines", [], 400)
        ...
        return res("Contra entry submitted", {
            "id":             entry.id,
            "workflowStatus": entry.workflow_status,
        }, 200)
...
def approve_contra_entry(entry_id, approved_by, comments=None):
    try:
        entry = ContraEntryMaster.query.get(entry_id)
        if not entry:
            return res("Contra entry not found", [], 404)
        if not entry.workflow_status.startswith("Pending"):
            return res("Contra entry is not pending approval", [], 400)
        ...
        return res("Contra entry approved", {
            "id":             entry.id,
            "workflowStatus": entry.workflow_status,
        }, 200)
...
def reback_contra_entry(entry_id, reback_by, comments=None):
    try:
        entry = ContraEntryMaster.query.get(entry_id)
        if not entry:
            return res("Contra entry not found", [], 404)
        if not entry.workflow_status.startswith("Pending"):
            return res("Contra entry is not pending", [], 400)
        if not comments:
        ...
        return res("Contra entry sent for correction", {
            "id":             entry.id,
            "workflowStatus": entry.workflow_status,
        }, 200)
...
def reject_contra_entry(entry_id, rejected_by, comments=None):
    try:
        entry = ContraEntryMaster.query.get(entry_id)
        if not entry:
            return res("Contra entry not found", [], 404)
        ...
        return res("Contra entry rejected", {
            "id":             entry.id,
            "workflowStatus": entry.workflow_status,
        }, 200)
...
def get_contra_entry_history(entry_id):
    try:
        entry = ContraEntryMaster.query.get(entry_id)
        if not entry:
            return res("Contra entry not found", [], 404)
...
def get_contra_entry_my_approval_status(entry_id, user_id):
    try:
        entry = ContraEntryMaster.query.get(entry_id)
        if not entry:
            return res("Contra entry not found", [], 404)
...
# ══════════════════════════════════════════════════════════════════
# 11. UUID LOOKUP (public)
# ══════════════════════════════════════════════════════════════════

def get_contra_entry_by_uuid(entry_uuid):
    try:
        entry = ContraEntryMaster.query.filter_by(contra_uuid=entry_uuid).first()
        if not entry:
            return res("Contra entry not found", [], 404)
        return res("Contra entry details fetched", _build_detail_payload(entry), 200)
    except Exception as e:
