...
class DebitNoteMaster(db.Model):
    __tablename__ = "debit_note_master"

    id              = db.Column(db.Integer, primary_key=True)
    debit_note_no   = db.Column(db.String(50), nullable=False)
    debit_note_uuid = db.Column(db.String(36), unique=True, nullable=True, index=True)
    ...
    credit_note_no   = db.Column(db.String(200), nullable=True)
    ...
    project   = db.relationship("Project",      backref="debit_notes")
    items     = db.relationship("DebitNoteItem", backref="debit_note", cascade="all,delete-orphan")
    gst_lines = db.relationship("DebitNoteGst",  backref="debit_note", cascade="all,delete-orphan")
...
class DebitNoteItem(db.Model):
    ...
    id            = db.Column(db.Integer, primary_key=True)
    debit_note_id = db.Column(db.Integer, db.ForeignKey("debit_note_master.id"), nullable=False)
...
class DebitNoteGst(db.Model):
    __tablename__ = "debit_note_gst"
