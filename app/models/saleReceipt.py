...
class SaleReceiptMaster(db.Model):
    ...
    # Financials (sum of current_amount — this receipt only)
    ...
    og_so          = db.relationship("OgSaleOrderMaster", backref="sale_receipts")
    certified_bill = db.relationship("BillingMaster",     backref="sale_receipts")
    ...
    items          = db.relationship("SaleReceiptItem", backref="receipt", cascade="all,delete-orphan")
    gst_lines      = db.relationship("SaleReceiptGst",  backref="receipt", cascade="all,delete-orphan")
...
class SaleReceiptItem(db.Model):
    """BASIC section — one row per CC Code with Booked/Received/Balance/Current."""

    __tablename__ = "sale_receipt_items"

    id         = db.Column(db.Integer, primary_key=True)
    receipt_id = db.Column(db.Integer, db.ForeignKey("sale_receipt_master.id"), nullable=False)
    ...
    booked_amount   = db.Column(db.Numeric(14, 2), default=0)   # from certified bill
    received_amount = db.Column(db.Numeric(14, 2), default=0)   # sum of prior receipts snapshot
    balance_amount  = db.Column(db.Numeric(14, 2), default=0)   # booked - received
    current_amount  = db.Column(db.Numeric(14, 2), default=0)   # this receipt entry
