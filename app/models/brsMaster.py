# app/models/brsMaster.py
# BRS — BRR Billing SRN (billing raised through BRR against SRN items)

from app.extensions import db
from datetime import datetime


class BrsMaster(db.Model):

    __tablename__ = "brs_master"

    id = db.Column(db.Integer, primary_key=True)

    brs_no   = db.Column(db.String(50), unique=True, nullable=False)
    brs_date = db.Column(db.Date, nullable=False)
    ...
    # Project-work order (SRN order type)
    order_id = db.Column(
        db.Integer,
    ...
    # Relationships
    project  = db.relationship("Project",                  backref="brs_list")
    vendor   = db.relationship("Vendor",                   backref="brs_list")
    order    = db.relationship("ProjectWorkOrderMaster",   backref="brs_list")
    brr      = db.relationship("BrrMaster",                backref="brs_billings")
...
class BrsItem(db.Model):

    __tablename__ = "brs_items"

    id = db.Column(db.Integer, primary_key=True)

    brs_id = db.Column(
        db.Integer,
        db.ForeignKey("brs_master.id"),
        nullable=False
    )
